...
export default function MembersTable() {
  ...
  const handleAcceptMember = async (id, experienceLevel) => {
    ...
    setMembers((pendingMembers) =>
      pendingMembers.map((member) => (member.id === updatedMember.id ? updatedMember : member)),
    );
  };
              ...
              {members.map((member) => (
                <tr key={member.id} className="group">
                  ...
                  <td className="whitespace-nowrap bg-white px-4 py-5 text-sm">
                    <select onChange={(e) => updateExperienceLevel(e, member.id)} value={member.experienceLevel}>
